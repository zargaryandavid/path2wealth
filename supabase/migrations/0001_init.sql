-- Profiles + transactions (already applied via SQL Editor; kept here for version control).
create table if not exists public.profiles (
  id uuid primary key references auth.users(id) on delete cascade,
  age text, gender text, income numeric, location text, specialty text, goal text,
...
create table if not exists public.transactions (
  ...
  amount numeric not null, category text not null, note text,
  recurring boolean default false, repeat_day int, repeat_months int,
